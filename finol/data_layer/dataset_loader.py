...
class DatasetLoader:
    ...
    def access_data(self, folder_path: str) -> List[Tuple[pd.DataFrame, str]]:
        """
        ...
        """
        # Get the initial list of all files in the directory
        file_names = sorted(os.listdir(folder_path))

        raw_files = []
        for index, file_name in enumerate(tqdm(file_names, desc="Data Loading")):
            if file_name.endswith(".xlsx") or file_name.endswith(".xls"):
                file_path = os.path.join(folder_path, file_name)
                try:
                    dataframe = pd.read_excel(file_path)
                    raw_files.append((dataframe, file_name))
    ...
    def feature_engineering(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str], Dict[str, int]]:
        ...
        _ = pd.concat([df["DATE"], ohlcv_features_df, overlap_features_df, momentum_features_df, volume_features_df,
                       cycle_features_df, price_features_df, volatility_features_df, pattern_features_df], axis=1)
    ...
    def plot_single_candlestick(self, index, row):
        ...
        WINDOW_SIZE = self.config["DATA_AUGMENTATION_CONFIG"]["IMAGE_DATA"]["WINDOW_SIZE"]
        ...
        pid = os.getpid()
        filename = f"stock_plot_{pid}.png"

        mpf.plot(candlestick_data, type="candle", title=f"Candlestick for {index}", volume=True, savefig=filename)
        image = Image.open(filename)

        # Resize the image
        image = image.resize((self.config["DATA_AUGMENTATION_CONFIG"]["IMAGE_DATA"]["SIDE_LENGTH"],
                              self.config["DATA_AUGMENTATION_CONFIG"]["IMAGE_DATA"]["SIDE_LENGTH"]), Image.LANCZOS)
    ...
    def process_single_asset(self, args: Tuple[int, pd.DataFrame, dict]) -> Dict:
        """
        Process a single asset's data. This function is designed to be used in parallel processing.

        :param args: Tuple containing (index, dataframe, config)
        :return: Dictionary containing processed data for this asset
        """
        i, df, asset_name, config = args
        # print(i, df, config)

        # Store original config in instance
        self.config = config
        ...
        # Extract test period labels for this asset for saving to .mat in the main process.
        test_start = pd.to_datetime(config["DATASET_SPLIT_CONFIG"][config["DATASET_NAME"]]["TEST_START_TIMESTAMP"])
        test_end = pd.to_datetime(config["DATASET_SPLIT_CONFIG"][config["DATASET_NAME"]]["TEST_END_TIMESTAMP"])
        df_label_temp = df_label[(df_label.index >= test_start) & (df_label.index <= test_end)]

        # Store as a 1D numpy array (chronologically), which will be column-stacked by the main process.
        test_label_matlab_series = df_label_temp["LABEL"].values
        ...
        result = {
            ...
            # "train_label": torch.from_numpy(train_label["LABEL"].values),
            # "val_label": torch.from_numpy(val_label["LABEL"].values),
            # "test_label": torch.from_numpy(test_label["LABEL"].values),
            "test_label_matlab_series": test_label_matlab_series,
        }

        if not config["DATA_AUGMENTATION_CONFIG"]["IMAGE_DATA"]["INCLUDE_IMAGE_DATA"]:
            ...
            result['train_data'] = torch.from_numpy(train_normalization.values)
            result['val_data'] = torch.from_numpy(val_normalization.values)
            result['test_data'] = torch.from_numpy(test_normalization.values)

        elif config["DATA_AUGMENTATION_CONFIG"]["IMAGE_DATA"]["INCLUDE_IMAGE_DATA"]:
            train_image_tensors = []
            val_image_tensors = []
            test_image_tensors = []
            ...
            result['train_data'] = torch.stack(train_image_tensors)
            result['val_data'] = torch.stack(val_image_tensors)
            result['test_data'] = torch.stack(test_image_tensors)

        result['train_label'] = torch.from_numpy(train_label["LABEL"].values)
        result['val_label'] = torch.from_numpy(val_label["LABEL"].values)
        result['test_label'] = torch.from_numpy(test_label["LABEL"].values)
    ...
    def load_dataset(self) -> Dict:
        ...
        self.config["LOAD_LOCAL_DATALOADER"] = True
        ...
        if not self.config["LOAD_LOCAL_DATALOADER"]:
            ...
            if len(raw_files) > 0:
                ...
                process_args = [(i, df, fname, self.config) for i, (df, fname) in enumerate(raw_files)]

                # Determine number of processes to use
                num_processes = min(mp.cpu_count(), len(raw_files))
                print(f"Using {num_processes} processes for parallel processing")

                # Process assets in parallel
                results = []
                with ProcessPoolExecutor(max_workers=num_processes) as executor:
                    # Create a new instance of DatasetLoader for each process
                    futures = []
                    for args in process_args:
                        future = executor.submit(DatasetLoader().process_single_asset, args)
                ...
                # Print the finalized asset sequence
                ordered_assets = [r.get('asset_name', f'asset_{r["index"]}') for r in results]
                print("Ordered assets:", ordered_assets)

                # Extract data from results
                DETAILED_FEATURE_LIST = None
                DETAILED_NUM_FEATURES = None

                mat_cols = []

                for result in results:
                    ds_train.append(result['train_data'])
                    ds_val.append(result['val_data'])
                    ds_test.append(result['test_data'])
                    label_train.append(result['train_label'])
                    label_val.append(result['val_label'])
                    label_test.append(result['test_label'])

                    # Collect the 1D sequence of test period labels for this asset
                    mat_cols.append(result["test_label_matlab_series"])

                    # Use the first asset's feature information
                    if DETAILED_FEATURE_LIST is None:
                        DETAILED_FEATURE_LIST = result['DETAILED_FEATURE_LIST']
                        DETAILED_NUM_FEATURES = result['DETAILED_NUM_FEATURES']

                # Save the .mat file in the main process
                import numpy as np
                from scipy import io

                if len(mat_cols) > 0:
                    df_label_MATLAB = np.column_stack(mat_cols)  # shape: [num_test_periods, num_assets]
                    dataset_name = self.config["DATASET_NAME"]
                    io.savemat(f"price_relative_{dataset_name}.mat", {"data": df_label_MATLAB})
            ...
            print(ROOT_PATH + "/data/datasets/" + self.config["DATASET_NAME"] + "_load_dataset_output.pt")
