...
#     import random
#     import os
#     # 为CPU、GPU、numpy和python内置的随机数生成器设置种子
...
#
#     # 3. (最关键) 强制PyTorch使用确定性算法
#     #    这会处理像 atomicAdd 这样的问题
#     #    注意: 这可能需要较新版本的PyTorch
#     torch.use_deterministic_algorithms(True)
#
#     # 如果上面一行报错 (老版本PyTorch)，可以尝试用下面这行代替
...
# import random
# import numpy as np
# def set_seed(seed=MANUAL_SEED):  # torch.manual_seed(3407) is all u need
#     os.environ['PYTHONHASHSEED'] = str(seed)
#     os.environ['CUBLAS_WORKSPACE_CONFIG'] = ':4096:8'
#     random.seed(seed)
#     np.random.seed(seed)
...
_CHECKPOINT_FOR_DOC = "google/stb-roberta-base"
_CONFIG_FOR_DOC = "STBConfig"
_TOKENIZER_FOR_DOC = "STBTokenizer"

_TRIVIA_QA_MAPPING = {
    "stb_attention": "attention/self",
...
class STBSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in STBModel forward() function)
...
class STBBlockSparseAttention(nn.Module):
    def __init__(self, config, seed=MANUAL_SEED):
        super().__init__()
    ...
    def forward(
        ...
        # torch.Size([0, 1, 256, 17]) torch.Size([29, 1, 256, 17]) torch.Size([29, 1, 256, 17])
        query_layer = self.transpose_for_scores(self.query(hidden_states))
        key_layer = self.transpose_for_scores(self.key(hidden_states))
        value_layer = self.transpose_for_scores(self.value(hidden_states))

        context_layer, attention_probs = self.stb_block_sparse_attention(
            query_layer,
            key_layer,
            value_layer,
            band_mask,
    ...
    def stb_block_sparse_attention(
            self,
            query_layer,  # Q, K, and V are input
            key_layer,
            value_layer,
            band_mask,
            from_mask,
            to_mask,  # to_mask is input
            from_blocked_mask,
            to_blocked_mask,
            n_heads,
            n_rand_blocks,
        ...
        # STB block-sparse attention as suggested in paper
        ...
        # print(from_seq_len, from_block_size, to_seq_len, to_block_size) -> 100 4 100 4
        if from_seq_len // from_block_size != to_seq_len // to_block_size:
            raise ValueError("Error the number of blocks needs to be same!")

        rsqrt_d = 1 / math.sqrt(attention_head_size)  # attention_head_size
        bsz = batch_size  # 29: num_assets
        attn_mask_penalty = -10000.0

        # generate random attention and corresponding masks
        np.random.seed(seed)

        # from_seq_len: input window size
        if from_seq_len in [1024, 3072, 4096]:  # old plans used in paper
            rand_attn = [
                self._stb_block_rand_mask(
            ...
            rand_attn = self._stb_block_rand_mask_with_head(
        ...
        # if it's the last line, that is, the 5 th part, the multiplication here should be: q [n - 1) x (k[0], k[1], k[2]
        # , ...., k[n-1]), n-1 is the total length.

        # [bsz, n_heads, from_block_size, -1] x [bsz, n_heads, to_seq_len, -1] ==> [bsz, n_heads, from_block_size, to_seq_len]
        # blocked_query_matrix: [29, 1, 32, 8, 17] after blocking
        # blocked_query_matrix[:, :, 0]: [29, 1, 8, 17]
        # key_layer: [29, 1, 256, 17]
        first_product = self.torch_bmm_nd_transpose(blocked_query_matrix[:, :, 0], key_layer,
                                                    ndim=4)  # first_product: [29, 1, 8, 256]

        first_product = first_product * rsqrt_d
        first_product += (
                                     1.0 - to_mask) * attn_mask_penalty  # (1.0 - all ones tensor with shape [29, 1, 1, 256], which means no penalty
        first_attn_weights = nn.functional.softmax(
            first_product, dim=-1  # softmax over the last dimension
        )  # [bsz, n_heads, from_block_size, to_seq_len]

        # [bsz, n_heads, from_block_size, to_seq_len] x [bsz, n_heads, to_seq_len, -1] ==> [bsz, n_heads, from_block_size, -1]
        # first_context_layer: [29, 1, 8, 17]
        # value_layer: [29, 1, 256, 17]
        first_context_layer = self.torch_bmm_nd(first_attn_weights, value_layer, ndim=4)
        first_context_layer.unsqueeze_(2)  # [29, 1, 8, 17] ==> [29, 1, 1, 8, 17]
    ...
    def _get_rand_attn_plan(from_seq_length, from_block_size, num_rand_blocks):
        ...
        return plan_from_length, plan_num_rand_blocks  # [88, 256] [3, 0]

    @staticmethod
    def _stb_block_rand_mask(
    ...
    def _stb_block_rand_mask_with_head(
            self,
            from_seq_length,
            to_seq_length,
            from_block_size,
...
# Copied from transformers.models.bert.modeling_bert.BertSelfOutput with Bert->STB
class STBSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class STBAttention(nn.Module):
    def __init__(self, config, seed=MANUAL_SEED):
        ...
        if self.config.attention_type == "original_full":
            self.self = STBSelfAttention(config)
        elif self.config.attention_type == "block_sparse":
            self.self = STBBlockSparseAttention(config, seed)
        else:
            raise ValueError(
                f"attention_type can either be original_full or block_sparse, but is {self.config.attention_type}"
            )

        self.output = STBSelfOutput(config)
    ...
    def set_attention_type(self, value: str):
        ...
        if value == "original_full":
            ...
            attn_weights = STBSelfAttention(self.config)
        else:
            # copy all weights to new sparse attention class
            attn_weights = STBBlockSparseAttention(self.config, self.seed)
    ...
    def forward(
        ...
        if self.attention_type == "original_full":
            ...
            if encoder_hidden_states is not None:
                raise ValueError("STB cannot be used as a decoder when config.attention_type != 'original_full'")
            self_outputs = self.self(
...
# Copied from transformers.models.bert.modeling_bert.BertIntermediate with Bert->STB
class STBIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
# Copied from transformers.models.bert.modeling_bert.BertOutput with Bert->STB
class STBOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class STBLayer(nn.Module):
    def __init__(self, config, seed=MANUAL_SEED):
        ...
        self.seq_len_dim = 1
        self.attention = STBAttention(config, seed=seed)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise TypeError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = STBAttention(config)
        self.intermediate = STBIntermediate(config)
        self.output = STBOutput(config)
...
class STBEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [STBLayer(config, seed=MANUAL_SEED) for layer_idx in range(config.num_hidden_layers)]
        )
        # print(self.layer)
...
STB_START_DOCSTRING = r"""
"""

STB_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`torch.LongTensor` of shape `({0})`):
            Indices of input sequence tokens in the vocabulary.

            Indices can be obtained using [`STBTokenizer`]. See [`PreTrainedTokenizer.encode`] and
...
@add_start_docstrings(
    "The bare STB Model transformer outputting raw hidden-states without any specific head on top.",
    STB_START_DOCSTRING,
)
class STBModel(PreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = STBEmbeddings(config)
        self.encoder = STBEncoder(config)
        ...
        if self.attention_type != "original_full" and config.add_cross_attention:
            logger.warning(
                "When using `STBForCausalLM` as decoder, then `attention_type` must be `original_full`. Setting"
    ...
    @add_start_docstrings_to_model_forward(STB_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @add_code_sample_docstrings(
    ...
    def forward(
        ...
        device = input_ids.device if input_ids is not None else inputs_embeds.device  # cuda:0
    ...
    def create_masks_for_block_sparse_attn(attention_mask: torch.Tensor, block_size: int):
        ...
        Args:
            attention_mask: 2D all ones Tensor of shape [batch_size, seq_length].
            block_size: int.
        ...
        # block_size = 16, seq_length = 256. attention_mask.shape = [6, 256]. blocked_encoder_mask.shape = [6, 16, 16]
        blocked_encoder_mask = attention_mask.view(batch_size, seq_length // block_size, block_size)
        band_mask = create_band_mask_from_inputs(blocked_encoder_mask,
                                                 blocked_encoder_mask)  # band_mask.shape = [6, 1, 12, 16, 48]

        from_mask = attention_mask.view(batch_size, 1, seq_length, 1)  # from_mask.shape = [6, 1, 256, 1]
        to_mask = attention_mask.view(batch_size, 1, 1, seq_length)  # to_mask.shape = [6, 1, 1, 256]
    ...
    def _pad_to_block_size(
        ...
        """
        A helper function to pad tokens and mask to work with implementation of STB block-sparse attention.
        """
        # padding
        ...
        if padding_len > 0:
            ...
            if position_ids is not None:
                # pad with position_id = pad_token_id as in modeling_stb.STBEmbeddings
