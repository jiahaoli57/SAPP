...
import matplotlib
matplotlib.use('Agg')  # 关键！必须在导入 pyplot 之前设置
            ...
            if (e + 1) % self.config["SAVE_EVERY"] == 0:
                with torch.no_grad():
                    ...
                    if val_loss < best_val_loss:
                        ...
                        epochs_no_improve = 0
                        # print(f"\nEpoch {e + 1}: Validation loss improved to {best_val_loss:.6f}. Saving model.")
                    else:
                        epochs_no_improve += 1
                        # print(f"\nEpoch {e + 1}: Validation loss did not improve. Patience: {epochs_no_improve}/{patience}.")
