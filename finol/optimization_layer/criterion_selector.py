...
class CriterionSelector:
    ...
    def __init__(self) -> None:
        ...
        self.criterion_dict = {
            ...
            "LogWealthL2Smooth": self.compute_log_wealth_l2_smooth_loss,
            "LogWealthL2Volatile": self.compute_log_wealth_l2_volatile_loss,
            "L2Smooth": self.compute_l2_smooth_loss,
            "L2Volatile": self.compute_l2_volatile_loss,
    ...
    def compute_log_wealth_l2_smooth_loss(self, portfolios: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        """
        Compute the ``LogWealthL2Smooth`` loss, which extends the ``LogWealth`` loss by incorporating diversification measures.
        ...
        :param portfolios: Portfolio weights tensor of shape (batch_size, num_assets).
        :param labels: Label tensor representing asset returns of shape (batch_size, num_assets).
        :return: ``LogWealthL2Smooth`` loss tensor, a modified version of ``LogWealth`` loss with diversification metrics included.
        """
        daily_returns = torch.sum(portfolios * labels, dim=-1)
        log_returns = torch.log(torch.clamp(daily_returns, min=1e-6))
    ...
    def compute_log_wealth_l2_volatile_loss(self, portfolios: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        """
        Compute the ``LogWealthL2Volatile`` loss, which extends the ``LogWealth`` loss by incorporating concentration measures.

        :param portfolios: Portfolio weights tensor of shape (batch_size, num_assets).
        :param labels: Label tensor representing asset returns of shape (batch_size, num_assets).
        :return: ``LogWealthL2Volatile`` loss tensor, a modified version of ``LogWealth`` loss with concentration metrics included.
        """
        daily_returns = torch.sum(portfolios * labels, dim=-1)
        log_returns = torch.log(torch.clamp(daily_returns, min=1e-6))
    ...
    def compute_l2_smooth_loss(self, portfolios: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        """
        Compute the ``L2Smooth`` loss, which measures the diversification of a portfolio based on the L2 norm of consecutive portfolio weight differences.

        This loss function calculates the mean L2 norm of the differences between consecutive portfolio weight vectors.

        :param portfolios: Portfolio weights tensor of shape (batch_size, num_assets).
        :param labels: Label tensor representing asset returns of shape (batch_size, num_assets).
        :return: ``L2Smooth`` loss tensor, indicating the degree of diversification based on the L2 norm of portfolio weight differences.
        """
        # Calculate the similarity loss
        # We use the L2 norm to measure the difference between consecutive predictions
        diff = portfolios[1:, :] - portfolios[:-1, :]
    ...
    def compute_l2_volatile_loss(self, portfolios: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        """
        ...
        :param portfolios: Portfolio weights tensor of shape (batch_size, num_assets).
        :param labels: Label tensor representing asset returns of shape (batch_size, num_assets).
        :return: ``L2Volatile`` loss tensor, indicating the degree of concentration based on the negative mean L2 norm of portfolio weight differences.
        """
        # Calculate the similarity loss
        # We use the L2 norm to measure the difference between consecutive predictions
        diff = portfolios[1:, :] - portfolios[:-1, :]
